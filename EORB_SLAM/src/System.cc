    ...
    mbDeactivateLocalizationMode(false), mpSensor(std::move(pSensor))
{
    ...
    mpMapDrawer = new MapDrawer(mpAtlas, pViewerParams);
    if (!pViewerParams) {
        LOG(WARNING) << "ViewerParams is null. Using default MapDrawer settings.";
    }
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    if (!mpTracker) {
        cerr << "❌ ERROR: `mpTracker` is NULL! Cannot save trajectory." << endl;
        return;
    }

    if (!mpAtlas) {
        cerr << "❌ ERROR: `mpAtlas` is NULL! Cannot save trajectory." << endl;
        return;
    }
    ...
    try {
        ...
        cerr << "❌ EXCEPTION in SaveTrajectoryTUM(): " << e.what() << endl;
    }
    catch (...) {
        cerr << "❌ UNKNOWN ERROR in SaveTrajectoryTUM()!" << endl;
    }
}
