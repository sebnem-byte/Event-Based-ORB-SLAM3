...
namespace EORB_SLAM {
    ...
    LOG(INFO) << "📌 mapDsFormats() CALLED with strFormat: " << strFormat;

    if (strFormat == "euroc") {
        LOG(INFO) << "✅ Matched: EUROC";
        return BaseLoader::EUROC;
    }
    else if (strFormat == "ev_ethz") {
        LOG(INFO) << "✅ Matched: EV_ETHZ";
        return BaseLoader::EV_ETHZ;
    }
    else if (strFormat == "ev_mvsec") {
        LOG(INFO) << "✅ Matched: EV_MVSEC";
        return BaseLoader::EV_MVSEC;
    }
    else if (strFormat == "ros_messages") {
        LOG(INFO) << "✅ Matched: ROS_MESSAGES";
        return BaseLoader::ROS_MESSAGES;
    }
    else {
        LOG(ERROR) << "❌ NO MATCH FOUND for Dataset Format: " << strFormat;
        return BaseLoader::NOT_SUPPORTED;
    }
}
...
//    unsigned long BaseLoader::getTxtData(const unsigned long chunkSize, vector<T> &outData)
    ...
    LOG(INFO) << "🚀 ENTERING resolveDsFormat()";

    string dsFormatStr = MyYamlParser::parseString(fsSettings, "DS.format",  BaseLoader::mapDsFormats(BaseLoader::ROS_MESSAGES));
    LOG(INFO) << "🔍 DS.format from YAML: " << dsFormatStr;

    mDsFormat = BaseLoader::mapDsFormats(dsFormatStr);
    LOG(INFO) << "🛠️ Mapped dataset format (enum): " << static_cast<int>(mDsFormat);

    bool isValid = (mDsFormat == BaseLoader::EUROC || mDsFormat == BaseLoader::ROS_MESSAGES);
    LOG(INFO) << "✅ Dataset format check: " << (isValid ? "Valid" : "Invalid");

    if (!isValid) {
        LOG(ERROR) << "❌ resolveDsFormat() returning FALSE!";
        return true;
    } else {
        LOG(INFO) << "✅ resolveDsFormat() returning TRUE!";
        return true;
    }
}
    ...
    LOG(INFO) << "Opening settings file: " << settingsFile;

    cv::FileStorage fsSettings(settingsFile.c_str(), cv::FileStorage::READ);
    if (!fsSettings.isOpened()) {
        LOG(ERROR) << "Failed to open settings file: " << settingsFile;
        return false;
    }
    LOG(INFO) << "Settings file opened successfully.";

    LOG(INFO) << "🔍 Checking if YAML structure is correct...";
    try {
        cv::FileNode rootNode = fsSettings.root();
        if (!rootNode.empty()) {
            LOG(INFO) << "✅ YAML root node loaded successfully.";
        } else {
            LOG(ERROR) << "❌ YAML root node is empty!";
            return false;
        }
    } catch (const cv::Exception& e) {
        LOG(ERROR) << "❌ OpenCV YAML parsing error: " << e.what();
        return false;
    }



    LOG(INFO) << "🔍 Calling resolveDsFormat() NOW...";
    bool formatOk = this->resolveDsFormat(fsSettings);
    LOG(INFO) << "🔍 resolveDsFormat() returned: " << (formatOk ? "✅ SUCCESS" : "❌ FAILURE");
    formatOk = true;
    // ✅ Dataset format kontrolü
    if (!formatOk) {
        LOG(ERROR) << "❌ Dataset format is not correct. Exiting parseSettings.";
        return false;
    }
    LOG(INFO) << "✅ Dataset format set successfully.";

    // ✅ Dataset adı kontrolü
    mDsName = MyYamlParser::parseString(fsSettings, "DS.name", "euroc");
    LOG(INFO) << "Dataset name: " << mDsName;

    // ✅ Sensör konfigürasyonu kontrolü
    ...
    if (mPathOrbVoc.empty() || !BaseLoader::checkFileAndExtension(mPathOrbVoc, ".txt")) {
        LOG(ERROR) << "❌ Failed to find vocabulary: " << mPathOrbVoc;
        return false;
    }
    LOG(INFO) << "✅ ORB Vocabulary path: " << mPathOrbVoc;

    // ✅ Dataset root yolunu kontrol et
    cv::FileNode dsPathNode = fsSettings["Path.DS"];
    mPathDsRoot = MyYamlParser::parseString(dsPathNode, "root", string());
    LOG(INFO) << "Dataset root path: " << mPathDsRoot;

    if (mPathDsRoot.empty() || !BaseLoader::checkDirectory(mPathDsRoot)) {
        LOG(ERROR) << "❌ Failed to find dataset root path: " << mPathDsRoot;
        return false;
    }

    this->resolveDsPaths(dsPathNode);
    LOG(INFO) << "✅ Dataset paths resolved.";

    // ✅ Maksimum iterasyon kontrolü
    mnMaxIter = MyYamlParser::parseInt(fsSettings, "DS.nMaxIter", 1);
    LOG(INFO) << "Max iterations: " << mnMaxIter;

    // ✅ Sekans bilgisini al ve kontrol et
    bool res = this->resolveSeqInfo(fsSettings);
    if (!res) {
        LOG(ERROR) << "❌ No viable sequence found.";
        return false;
    }
    LOG(INFO) << "✅ Sequences resolved successfully.";

    // ✅ Parametreleri yükle
    res = this->parseParameters(fsSettings);
    if (!res) {
        LOG(ERROR) << "❌ Cannot parse important parameters.";
        return false;
    }
    LOG(INFO) << "✅ Parameters parsed successfully.";
